...
impl LLamaParams<f32> {
    pub fn from_safetensors(safetensor: &SafeTensors, config: &LlamaConfigJson) -> Self {
        ...
        // safetensors会只存储一份数据，我们测试用的story模型就是这样。作业阶段你可以只关心
        // story模型，但是后续项目中你需要处理两个矩阵不同的情况。
       
        //let get_tensor: impl Fn(&str) -> Tensor<f32> = |name: &str| {
        let get_tensor = |name:&str| {
            let ts = safetensor.tensor(name).unwrap();
            let shape = ts.shape();
            //let data = ts.data().iter().cloned().map(|x| x as f32).collect();
            let data:Vec<f32> = ts.data().chunks_exact(4).map(|chunk|f32::from_le_bytes(chunk.try_into().expect("转换失败"))).collect();
            let ts_get = Tensor::new(data,&shape.to_vec());
            //println!("{}: shape = {:?}  data = {:?}",name,ts_get.shape(),ts_get.data());
            ts_get
        };
        ...
        LLamaParams {
            ...
            rms_att_w:(0..layer).map(|x|get_tensor(&format!("model.layers.{}.input_layernorm.weight", x))).collect(),
